...
<style>
  ...
  .pill {
    ...
    border-radius: 2px;
    /*
     * Приподнятая плашка, а не акцентная заливка.
     *
     * В тёмной теме это одно и то же — там `--raised` и есть акцент. В светлой
     * они расходятся: шапка из чернильных плашек поперёк белого окна забирает
     * взгляд себе, вместо того чтобы отдать его написанному в ней. Там плашка
     * светлее полосы, на которой лежит, и этой ступенью от неё и отделяется.
     */
  ...
  /*
   * Маскот берёт цвет заливок, а не краски текста.
   *
   * В светлой теме это тёмный рисунок на серой полосе: белый на ней терялся,
   * а подложка под ним — круг, плашка, что угодно — читалась как посторонний
   * предмет в шапке. Заодно на маскота распространяется цвет системы: он часть
   * того же акцента, что и плашки вокруг.
   */
  .mascot-slot {
    position: absolute;
    right: 12px;
